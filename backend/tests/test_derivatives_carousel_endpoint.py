"""Endpoint tests for POST /api/v1/derivatives/carousel (P1-5 slice 2, Task 3).

Mirrors `test_derivatives_card_endpoint.py`'s harness: the no-DB `client`
fixture for the BYOK/schema/managed-ineligible paths, and a real-Postgres
`TestClient(app)` lifespan for the trust-section access gate + the
managed-entitlement fork (neither `require_project_access` nor
`resolve_managed_access` is stubbed). `compile_carousel_png` is always
stubbed — rendering itself is Task 1's concern.
"""
...
_GOOD_CAROUSEL = json.dumps(
    {"frames": [{"headline": f"Headline {i}", "subtext": f"Subtext {i}"} for i in range(1, 6)]}
)
_FIVE_PNGS = [f"PNG{i}".encode() for i in range(1, 6)]
...
async def test_source_text_ok_and_key_never_leaks(client, known_test_api_key, caplog):
    with (
        patch(
            ...
            return_value=fake_provider(text=_GOOD_CAROUSEL),
        ),
        patch(
            "backend.src.derivatives.router.compile_carousel_png",
            return_value=_FIVE_PNGS,
        ),
    ):
        r = await client.post(
            "/api/v1/derivatives/carousel",
    ...
    for i, frame in enumerate(body["frames"]):
        assert frame["card"]["headline"] == f"Headline {i + 1}"
        assert frame["image_png_base64"] == base64.b64encode(_FIVE_PNGS[i]).decode()
        # Flat source_text -> no provenance claim on any frame.
        assert frame["card"]["source_label"] is None
...
async def test_managed_ineligible_400(client):
    r = await client.post("/api/v1/derivatives/carousel", json={"source_text": "s"})
...
def _isolate_managed_db_state():
    ...
    test_derivatives_card_endpoint.py's `_isolate_managed_db_state`)."""
...
@_managed_db
def test_topic_version_reviewer_200_last_frame_only_provenance(monkeypatch):
    ...
    for frame in frames[:-1]:
        assert frame["card"]["source_label"] is None
    assert frames[-1]["card"]["source_label"] == "Based on 2 cited source(s)"
    ...
    with TestClient(app) as c:
        ...
        try:
            with (
                ...
                r = c.post("/api/v1/derivatives/carousel", json={"source_text": "s"})
        finally:
            app.dependency_overrides.pop(optional_user, None)
    assert r.status_code != 400
...
def test_managed_over_cap_429(monkeypatch):
    ...
    with TestClient(app) as c:
        app.dependency_overrides[optional_user] = lambda: _principal(sub, email)
        try:
            mock_generate_carousel = MagicMock()
            with patch("backend.src.derivatives.router.generate_carousel", mock_generate_carousel):
                r = c.post("/api/v1/derivatives/carousel", json={"source_text": "s"})
        finally:
            app.dependency_overrides.pop(optional_user, None)
    assert r.status_code == 429
    mock_generate_carousel.assert_not_called()
