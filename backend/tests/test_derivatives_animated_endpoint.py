"""Endpoint tests for POST /api/v1/derivatives/animated (P1-5 P3).
...
`resolve_managed_access` is stubbed). `compile_animated_gif` is always
stubbed — rendering itself is Task 1's (the compiler's) concern.
"""
...
async def test_source_text_ok_and_key_never_leaks(client, known_test_api_key, caplog):
    with (
        ...
        patch(
            "backend.src.derivatives.router.compile_animated_gif",
            return_value=_GIF_BYTES,
        ),
    ):
        r = await client.post(
            "/api/v1/derivatives/animated",
            json={"source_text": "Stormwater.", "api_key": known_test_api_key},
        )
    assert r.status_code == 200
    body = r.json()
    assert body["card"]["headline"] == "Stormwater, decoded"
    assert body["preset"] == "fade"
    assert body["provenance"] == "ai-generated"
    assert body["image_gif_base64"] == base64.b64encode(_GIF_BYTES).decode()
...
async def test_malformed_topic_version_id_422(client, known_test_api_key):
    # `topic_version_id` is typed `uuid.UUID | None` — a malformed value must
    # be rejected by pydantic with a clean 422 before the handler ever runs a
    # query against the uuid column, never an uncaught asyncpg DataError
    # surfacing as a framework 500.
    r = await client.post(
        "/api/v1/derivatives/animated",
...
async def test_invalid_preset_422(client, known_test_api_key):
    r = await client.post(
        "/api/v1/derivatives/animated",
        json={"source_text": "s", "preset": "spin", "api_key": known_test_api_key},
    )
    assert r.status_code == 422


async def test_managed_ineligible_400(client):
    r = await client.post("/api/v1/derivatives/animated", json={"source_text": "s"})
    ...
    with TestClient(app) as c:
        ...
        try:
            with (
                ...
                r = c.post("/api/v1/derivatives/animated", json={"source_text": "s"})
        finally:
            app.dependency_overrides.pop(optional_user, None)
    assert r.status_code != 400
