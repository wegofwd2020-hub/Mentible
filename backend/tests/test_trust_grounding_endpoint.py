"""TDD for the grounding-check endpoints + `version_grounding` persistence
(P1-4 T4): `POST .../artifacts/versions/{version_id}/grounding-check` and
`POST .../topic-versions/{version_id}/grounding-check` submit an async job
(mirrors `generate_version`'s eligibility/envelope/enqueue shape — owner-only,
billable) whose worker (`grounding_check_task`) upserts a stored report into
`version_grounding`. `GET /versions/{id}` / `GET /topic-versions/{id}` then
surface that stored report (+ a `stale` flag) as `quality.grounding`.
"""
...
from backend.src.trust import grounding_repo, project_repo
...
def _artifact_version_with_source(c):
    """Create a project + one source + one artifact version citing it.

    Returns (project_id, version_id, input_id)."""
...
def test_owner_submits_grounding_check_returns_202_and_enqueues():
    with TestClient(app) as c:
        ...
        with patch("backend.src.trust.router.grounding_check_task.delay") as mock_delay:
            r = c.post(
                f"/api/v1/trust/artifacts/versions/{vid}/grounding-check",
                json={"api_key": key},
            )
...
def test_owner_submits_topic_grounding_check_returns_202_with_topic_kind():
    with TestClient(app) as c:
        owner = f"o-{uuid.uuid4()}"
        _as(owner, f"{owner}@x.z")
        pid = c.post("/api/v1/trust/projects", json={"title": "P", "topic": "t"}).json()["id"]
        ...
        with patch("backend.src.trust.router.grounding_check_task.delay") as mock_delay:
            r = c.post(
                f"/api/v1/trust/topic-versions/{tvid}/grounding-check",
...
def test_grounding_check_unknown_version_404():
    with TestClient(app) as c:
        ...
        _as(owner, f"{owner}@x.z")
        r = c.post(
            f"/api/v1/trust/artifacts/versions/{uuid.uuid4()}/grounding-check",
            json={"api_key": "sk-ant-" + "e" * 20},
        )
        assert r.status_code == 404


# ── Reviewer forbidden (billable → owner only) ───────────────────────────────


def test_reviewer_cannot_submit_grounding_check_403():
    with TestClient(app) as c:
        owner = f"o-{uuid.uuid4()}"
        _as(owner, f"{owner}@x.z")
        ...
        c.post("/api/v1/trust/session/sync")  # redeem invite -> reviewer membership
...
# ── Stored report surfaces on read, with a stale flag ────────────────────────
...
async def _upsert_report(version_id, version_kind, sources, cited_ids, *, model="m"):
    ...
    try:
        report = {
            ...
            "partial": 0,
            "unsupported": 0,
            "by_section": [{"section_index": 0, "claims": []}],
            "model": model,
        }
...
def test_stored_grounding_surfaces_on_read_and_flips_stale():
    with TestClient(app) as c:
        owner = f"o-{uuid.uuid4()}"
        _as(owner, f"{owner}@x.z")
        ...
        g = r.json()["quality"]["grounding"]
        assert g is not None
        assert g["stale"] is False
        assert "cited_content_hash" not in g  # popped before returning

        # The cited input's content_hash changes underneath the stored report.
        asyncio.run(_set_input_content_hash(iid, "sha256:changed"))

        r2 = c.get(f"/api/v1/trust/versions/{vid}")
        g2 = r2.json()["quality"]["grounding"]
        assert g2 is not None
        assert g2["stale"] is True


def test_no_stored_grounding_is_none():
    with TestClient(app) as c:
        owner = f"o-{uuid.uuid4()}"
        _as(owner, f"{owner}@x.z")
        _pid, vid, _iid = _artifact_version_with_source(c)
        ...
        r = c.get(f"/api/v1/trust/versions/{vid}")
        assert r.json()["quality"]["grounding"] is None
