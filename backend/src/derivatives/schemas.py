...
class AnimatedRequest(BaseModel):
    """Body of POST /derivatives/animated — generate an animated promotional card.

    Same `api_key` custody discipline as `CardRequest` (ADR-001): never
    logged, never persisted in plaintext. Exactly one of `source_text` /
    `topic_version_id` must be supplied, mirroring `CardRequest`. The copy is
    generated via the same `generate_card` call as `/card` (fixed "square"
    size) — `preset` only steers the compiler's animation, not the copy.
    """
...
class AnimatedResponse(BaseModel):
    """Body of a completed publish-animated-card generation."""
