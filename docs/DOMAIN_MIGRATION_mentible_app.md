...
- [x] **Docs sweep** (2026-08-20) — `README.md`, `CLAUDE.md`, `docs/DEPLOYMENT_PIPELINE.md`, `docs/user-guides/01-sign-in-with-google.md` updated: surface table split into canonical `mentible.app` + still-live legacy path, the "no standalone Mentible domain" Domains/DNS block rewritten to name `mentible.app` as canonical, getting-started guide leads with `mentible.app`. `STATUS.md`, `GO_LIVE.md`, `competitive-analysis`, `funnel` intentionally left — dated historical snapshots / `/mentible-api` runbook, all still accurate. Preserved throughout: `/mentible-api`, `/app/mentible`, `/demos/mentible`, marketing landing. Also updated the mambakkam-net web product links (siva-m bio + kaundinya row) → `https://mentible.app`.
