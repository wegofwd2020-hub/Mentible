...
- **D2 — Reuse KDP's raster steps (they also flatten animations).** For `epub2`, math → raster PNG and Mermaid diagrams → raster PNG, using the **exact same** `rasterizeMath`/`replaceMathWithImages` + `PrerenderedRasterDiagramRenderer`/`rasterizeDiagramPngs` KDP uses. A rastered diagram is a **static PNG** — so this *is* the "flatten animated SVG" requirement (no `<animate>`/SMIL survives). Generalize the relevant `profile === "kdp"` branches to `profile !== "default"` where the behavior is shared (raster math + raster diagrams); keep KDP-only branches (font-drop, JPEG cover, `dc:date` ISO, KDP stylesheet) gated on `=== "kdp"`.
