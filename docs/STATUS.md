...
| 034 | Persona-validation gates + claims-honesty principles | Proposed — 2026-07-15; design-only. Gates *messaging/GTM* per vertical, not what may be built (D5); D3's honesty principles (storage ≠ processing; hosted is not zero-knowledge) are non-negotiable and bind shipped copy |
| 035 | Hosted media sync (media slice 2 — attached figures follow the user, E2E-encrypted) | Proposed — 2026-07-16; design-only (not built). Closes the gap between ADR-033 D2 (scopes hosted content to `book.json`) and media slice 1 (bytes live *outside* `book.json`), which would otherwise sync figure refs without their bytes. Media is E2E-encrypted under ADR-014 D10's per-book DK — it **narrows** ADR-033 D4, since nothing indexes images. Private-per-user only (sharing fenced); depends on ADR-033's hosted library **and** ADR-014 D10's unbuilt passphrase flow. D5's copy superseded by ADR-036 D5 |
| 036 | Vision-assisted figure captions (media slice 3 — the first time a figure reaches a model) | Proposed — 2026-07-16; design-only (not built). Scope is deliberately the *smallest* slice that proves the image seam: author-in-the-loop caption/alt-text suggestion, never auto-applied, no index, no new storage. Device decrypts locally and sends through the existing ADR-001 passthrough — **ADR-035 D2's at-rest E2E is unweakened** (custody ≠ transient passthrough) and server-side vision stays foreclosed. Extends `wegofwd-llm`'s `LLMRequest` with optional image parts (additive → minor, ADR-012 D4); `Capabilities.vision` exists today but **no adapter ever sets it true**. Pinned provider without vision → fail loudly + explicit re-pin (ADR-016 D1/D3, no silent fallback). Falsifies the shipped "nothing is sent to the AI" copy → D5 replaces it and names the destination (ADR-034 D3.1/D3.2). ⚠ The alt-text **accessibility** bug (`alt=""` on uncaptioned figures = "decorative") is a *prerequisite*, fixable with a plain field and **no AI** — this ADR explicitly does not claim it |
