...
| `docs/adr/ADR-042-local-inference-ollama-consideration.md` | Before proposing local/self-hosted inference (Ollama or any GPU-hosted open model) as an engine — the decision is **no managed/self-hosted inference** (infra cost + ops + draft-grade quality don't beat Groq-free + Anthropic-BYOK; "the model is the commodity"), with local inference kept only as a **deferred, low-effort BYOK/privacy option for the native/desktop build** (Ollama as an `openai_compatible` registry entry at a user base URL; gated off web — a browser can't reach `localhost`). Accepted — direction only, no code; relates to ADR-005/014 |
